...
function buildPrompt({ kind, lead, input }: GenerateArgs) {
  if (kind === "proposal") {
    return [
      ...
      "Use the structure of a clean client proposal: Summary, Project Overview, Deliverables, Scope of Work, Estimated Cost, Additional Notes, Next Step.",
...
function buildFallback({ kind, lead, input }: GenerateArgs) {
  ...
  if (kind === "proposal") {
    return [
      ...
      "",
      "Summary",
      `Ghost AI Solutions will design and deploy a focused AI lead-follow-up workflow for ${company}. The goal is to recover missed ${niche.toLowerCase()} opportunities, route replies, and create booked-call visibility without replacing the client's existing tools.`,
      "",
      "Project Overview",
      `${company} appears to have a ${niche.toLowerCase()} lead-flow problem worth tightening before more demand is purchased. Current stage is ${lead?.stage || "active"} with an estimated opportunity value of $${Number(lead?.value || 7500).toLocaleString()}.`,
      "",
      "Deliverables",
      "01 Lead source/import setup and segment definition",
      "02 Contact scoring, dedupe, and suppression checks",
      "03 AI-assisted email/SMS approval queue",
      "04 Reply classification and booked-call routing",
      "05 Proposal follow-up, CRM sync, and attribution tracking",
      "",
      "Scope of Work",
      `Ghost AI Solutions will configure a 7-day pilot around one approved ${niche.toLowerCase()} segment, draft approved follow-up, monitor replies, and surface booked opportunities for operator review.`,
      "",
      "Estimated Cost",
      "Setup: $2,500",
      "Monthly optimization: $1,000/mo after pilot approval",
      "Optional upside share: 12% of recovered revenue where attribution is visible",
      "",
      "Additional Notes",
      "Client is responsible for provider costs, API usage, platform fees, and compliance approvals. Additional integrations or scope expansions may require separate approval.",
      "",
      "Next Step",
      "Approve one narrow segment, run the pilot for 7 days, and review booked calls plus pipeline value before expanding.",
    ].join("\n");
  }
