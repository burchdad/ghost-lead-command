...
export function ensureVegaOnboardingSchema(prisma: PrismaClient) {
...
const statements = [
  ...
  `CREATE INDEX IF NOT EXISTS "AutomationEvent_workspaceId_idx" ON "AutomationEvent"("workspaceId");`,
  `CREATE INDEX IF NOT EXISTS "AutomationEvent_leadId_idx" ON "AutomationEvent"("leadId");`,
  ...
  `DO $$
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AutomationEvent_workspaceId_fkey') THEN
      ALTER TABLE "AutomationEvent" ADD CONSTRAINT "AutomationEvent_workspaceId_fkey" FOREIGN KEY ("workspaceId") REFERENCES "Workspace"("id") ON DELETE CASCADE ON UPDATE CASCADE;
    END IF;
    IF to_regclass('public."Lead"') IS NOT NULL AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AutomationEvent_leadId_fkey') THEN
      ALTER TABLE "AutomationEvent" ADD CONSTRAINT "AutomationEvent_leadId_fkey" FOREIGN KEY ("leadId") REFERENCES "Lead"("id") ON DELETE SET NULL ON UPDATE CASCADE;
    END IF;
